...
class TSharp:
    ...
    def paket_yukle_ve_calistir(self, paket_adi):
    ...
    def interaktif_mod(self):
        print("TSharp 4.2 (varsayilan, May 25 2026)")
        ...
        while True:
            ...
            if blok_mu:
                ...
                # Tek satırlık komutlar ve Bilinmeyen Kelime Kontrolü
                try:
                    # Komutun ilk kelimesini al (Örn: "yasdır", "yazdır", "x")
                    parcalar = temiz.split()
                    ilk_kelime = parcalar[0] if parcalar else ""

                    # 1. Eğer bu bir değişken ataması DEĞİLSE (içinde '=' yoksa)
                    # 2. Ve bilinen bir anahtar kelime/fonksiyon DEĞİLSE
                    bilinenler = ['yazdir', 'yazdır', 'bekle', 'temizle', 'liste', 'sozluk', 'oku', 'sil'] 
                    # Not: Yukardaki listeye (bilinenler) tüm ana komutlarını ekleyebilirsin.

                    if "=" not in temiz and ilk_kelime not in bilinenler and ilk_kelime not in self.degiskenler:
                        print(f"Bilinmeyen anahtar kelime: <{ilk_kelime}>")
                    else:
                        # Bilinen bir şeyse veya atamaysa normal çalıştır
    ...
    def satirlari_calistir(self, satirlar):
        ...
        while i < len(satirlar):
            ...
            if satir == 'kullan ag':
                ...
                self.requests_session.headers.update({'User-Agent': 'TSharp/4.2'})
                i += 1
                continue
            ...
            if satir == 'kullan hash':
                ...
                i += 1
                continue
            # TPM: Kurulu kullanici paketleri
            if satir.startswith('kullan '):
                paket_adi = satir[len('kullan '):].strip()
                DAHILI_PAKETLER = {'gui6', 'gui', 'ag', 'oyun', 'sifrele', 'hash'}
                if paket_adi not in DAHILI_PAKETLER:
                    self.paket_yukle_ve_calistir(paket_adi)
                    i += 1
                    continue
    ...
    def komutu_calistir(self, satir):
        ...
        # ---- Atama ve fonksiyon çağrısı ----
            ...
            # Bilinmeyen komut - sessizce geç (ya da debug modunda yaz)
            pass
